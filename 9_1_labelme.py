...
Dataset_root = '/home/c/桌面/Triangle_215_Keypoint_Labelme'

# 检查路径是否存在
labelme_jsons_dir = os.path.join(Dataset_root, 'labelme_jsons')

if not os.path.exists(labelme_jsons_dir):
    print(f"错误：目录 '{labelme_jsons_dir}' 不存在！")
    exit(1)

os.chdir(labelme_jsons_dir)
json_files = os.listdir()
print(f'共有 {len(json_files)} 个 labelme 格式的 json 标注文件')

# --- 改动 1: 创建一个全局列表来存放所有行数据，而不是直接操作空 DataFrame ---
data_list = []

def process_single_labelme(labelme_path):
    ...
    for each_ann in labelme['shapes']: # 遍历每一个标注
        ...
        if each_ann['shape_type'] == 'polygon': # 筛选出多段线（polygon）标注
            ...
            img_bgr = cv2.imread('/home/c/桌面/Triangle_215_Keypoint_Labelme/images/' + imagePath)
       ...
       # --- 改动 2: 将字典存入列表，而不是直接 append 到 DataFrame ---
        if df_temp:
            data_list.append(df_temp)
# --- 改动 3: 循环结束后，一次性构造 DataFrame ---
for labelme_path in tqdm(json_files):
    process_single_labelme(labelme_path)

df = pd.DataFrame(data_list)

# 检查结果并保存
if not df.empty:
    #df.to_csv(os.path.join(Dataset_root, 'kpt_dataset_eda.csv'), index=False, encoding='utf-8-sig')
    df.to_csv('/home/c/桌面/Triangle_215_Keypoint_Labelme/kpt_dataset_eda.csv', index=False)
    print("\n处理完成,CSV 已保存。")
else:
    print("\n警告:未提取到任何有效数据。")
#df = pd.DataFrame()
